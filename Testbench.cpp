...
void Testbench::do_gauss() {
  ...
  for (y = (0 - yBound); y != (height + yBound + 100); ++y) {
    ...
    // check if at least three row of data has been sent before waiting for returned results
    if (++row_sent < 3)
      continue;
    wait(1);
    --row_sent;
    ...
    //total = i_result_r.read();
    //cout << "Now at " << sc_time_stamp() << endl; //print current sc_time
    for (x = 0 ; x != width; ++x) {
    *(target_bitmap + bytes_per_pixel * (width * (y-1) + x) + 2) = i_result_r.read();;
    *(target_bitmap + bytes_per_pixel * (width * (y-1) + x) + 1) = i_result_g.read();;
    *(target_bitmap + bytes_per_pixel * (width * (y-1) + x) + 0) = i_result_b.read();;
    }
  }
  sc_stop();
}
